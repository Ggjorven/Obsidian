...
namespace Nano::Graphics::Internal
{
	...
	CD3DX12_CPU_DESCRIPTOR_HANDLE Dx12Resources::Heap::CreateSRV(Format format, ImageDimension dimension, const ImageSubresourceSpecification& subresources, const ImageSpecification& specs, ID3D12Resource* resource)
	{
		...
		CD3DX12_CPU_DESCRIPTOR_HANDLE handle = GetNextHandle();
	...
	void Dx12Resources::Heap::Free(CD3DX12_CPU_DESCRIPTOR_HANDLE handle)
	{
		FreeEntries.emplace_back(1, handle);
	}
	...
	void Dx12Resources::Heap::Grow(uint32_t minNewSize)
	{
		...
		if constexpr (Information::Validation)
		{
			if (IsShaderVisible)
			{
				NG_ASSERT((newSize < ((Type == D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV) ? static_cast<uint32_t>(D3D12_MAX_SHADER_VISIBLE_DESCRIPTOR_HEAP_SIZE_TIER_1) : static_cast<uint32_t>(D3D12_MAX_SHADER_VISIBLE_SAMPLER_HEAP_SIZE))), "[Dx12Resources::Heap] Size of new descriptor heap exceeds maximum size.");
			}
		}

		ID3D12DescriptorHeap* oldHeap = DescriptorHeap;
		...
		m_Device.GetContext().GetD3D12Device()->CopyDescriptorsSimple(oldSize, Offset, oldHeap->GetCPUDescriptorHandleForHeapStart(), Type);

		// Copy the free entries but fix the handles to the new heap
		for (auto& entry : FreeEntries)
		{
			size_t oldOffsetFromStart = oldHeap->GetCPUDescriptorHandleForHeapStart().ptr - entry.Handle.ptr;
			entry.Handle = DescriptorHeap->GetCPUDescriptorHandleForHeapStart();
			entry.Handle.Offset(static_cast<INT>(oldOffsetFromStart));
		}
	}
	...
	// Private methods
	////////////////////////////////////////////////////////////////////////////////////
	CD3DX12_CPU_DESCRIPTOR_HANDLE Dx12Resources::Heap::GetNextHandle()
	{
		// If we have a freed slot use that else use the offset
		CD3DX12_CPU_DESCRIPTOR_HANDLE handle = {};
		if (!FreeEntries.empty())
		{
			Entry& e = FreeEntries.back();
