...
namespace Obsidian::Internal
{
			...
			for (auto descriptorLayout : layouts)
			{
				...
				if (!range.has_value())
				{
					for (const auto& item : vulkanLayout.GetBindingItems())
					{
						...
						OB_ASSERT((item.Size <= 128), "[VkxxxPipeline] Push constants size has to be less than 128 bytes.");
						OB_ASSERT((item.Size % 4 == 0), "[VkxxxPipeline] Push constants size must be aligned to 4 bytes.");
    ...
    {
		OB_ASSERT(specs.Input, "[VkGraphicsPipeline] No proper inputlayout specified.");
		OB_ASSERT(specs.Pass, "[VkGraphicsPipeline] No proper renderpass specified.");
		...
		OB_ASSERT(!vulkanRenderpass.GetFramebuffers().empty(), "[VkGraphicsPipeline] Renderpass passed in with no created framebuffers.");
	...
	VulkanComputePipeline::VulkanComputePipeline(const Device& device, const ComputePipelineSpecification& specs)
		: m_Specification(specs)
	{
		OB_ASSERT(specs.ComputeShader, "[VkComputePipeline] No compute shader passed in to compute pipeline.");
