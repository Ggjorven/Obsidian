...
namespace Obsidian::Internal
{
    ...
    QueueFamilyIndices QueueFamilyIndices::Find(VkSurfaceKHR surface, VkPhysicalDevice device)
	{
        ...
        OB_ASSERT(indices.CompletedQueues, "[VkDevice] Failed to query queues. Contact developer.");
	...
	SwapchainSupportDetails SwapchainSupportDetails::Query(VkSurfaceKHR surface, VkPhysicalDevice device)
	{
        ...
        OB_ASSERT((formatCount != 0), "[VkPhysicalDevice] GPU doesn't support any formats?");
        ...
        OB_ASSERT((formatCount != 0), "[VkPhysicalDevice] GPU doesn't support any presentmodes?");
    ...
    {
        OB_ASSERT(instance, "[VkPhysicalDevice] No valid instance passed in.");
        OB_ASSERT(instance, "[VkPhysicalDevice] No valid surface passed in.");

        uint32_t deviceCount;
        VK_VERIFY(vkEnumeratePhysicalDevices(instance, &deviceCount, nullptr));

        OB_ASSERT((deviceCount != 0), "[VkPhysicalDevice] Failed to find a GPU with Vulkan support!");
        ...
        OB_ASSERT(m_PhysicalDevice, "[VkPhysicalDevice] Failed to find a GPU with support for this application's required Vulkan capabilities!");
    }
    ...
    VkFormat VulkanPhysicalDevice::FindSupportedFormat(const std::vector<VkFormat>& candidates, VkImageTiling tiling, VkFormatFeatureFlags features) const
    {
        ...
        OB_LOG_ERROR("[VkPhysicalDevice] Failed to find supported format!");
		...
		: m_PhysicalDevice(physicalDevice)
	{
		...
		// dynamicRenderingFeature.sType = VK_STRUCTURE_TYPE_PHYSICAL_DEVICE_DYNAMIC_RENDERIOB_FEATURES_KHR;
