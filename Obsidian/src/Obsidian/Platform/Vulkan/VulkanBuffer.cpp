...
namespace Obsidian::Internal
{
        ...
        : m_Attributes(attributes.begin(), attributes.end())
    {
        ...
        {
            OB_ASSERT((spec.ArraySize > 0), "[VkInputLayout] A vertex attribute specification must be more than 0.");
            ...
            {
                OB_ASSERT((bindingMap[spec.BufferIndex].inputRate == (spec.IsInstanced ? VK_VERTEX_INPUT_RATE_INSTANCE : VK_VERTEX_INPUT_RATE_VERTEX)), "[VkInputLayout] All vertex attributes must have the same input rate.");
            }
        }
    ...
    {
        OB_ASSERT((m_Specification.IsDynamic) || (m_Specification.Size != 0), "[VkBuffer] Size must not equal 0.");
        ...
        if constexpr (Information::Validation)
        {
            if (m_Specification.IsIndexBuffer || m_Specification.IsTexel)
                OB_ASSERT((m_Specification.BufferFormat != Format::Unknown), "[VkBuffer] A Texel or Index buffer must not have BufferFormat unknown.");
        }
        ...
        // is to prevent unnecessary brances during runtime when they can be done during the same branch.
        {
            ...
            {
                OB_ASSERT(((m_Specification.BufferFormat == Format::R16UInt) || (m_Specification.BufferFormat == Format::R32UInt)), "[VkBuffer] An index buffer must have format R16 or R32 float, a uint16 or uint32.");
            ...
            OB_ASSERT(((m_Alignment & (m_Alignment - 1)) == 0), "[VkBuffer] Internal error: Alignment must be a power of 2.");
        }
        ...
        // Size helper
        {
            ...
            {
                OB_ASSERT((m_Specification.ElementCount != 0), "[VkBuffer] Element count must not equal zero when dynamic is enabled.");
