...
#elif !defined(OB_PLATFORM_APPLE)
...
namespace Obsidian::Internal
{
    ...
    {
        OB_PROFILE("VkAllocator::AllocateImage()");

        OB_ASSERT(m_Allocator, "[VkAllocator] Allocator not initialized.");
        OB_ASSERT((width > 0) && (height > 0), "[VkAllocator] Invalid width or height passed in for image allocation.");
    ...
    {
        OB_PROFILE("VkAllocator::DestroyImage()");

        OB_ASSERT(m_Allocator, "[VkAllocator] Allocator not initialized.");
        OB_ASSERT((image != VK_NULL_HANDLE), "[VkAllocator] Invalid image passed in.");
        OB_ASSERT((allocation != VK_NULL_HANDLE), "[VkAllocator] Invalid allocation passed in.");
    ...
    {
        OB_PROFILE("VkAllocator::MapMemory()");
    ...
    {
        OB_PROFILE("VkAllocator::SetData()");

		OB_ASSERT((allocation != VK_NULL_HANDLE), "[VkAllocator] Invalid allocation passed in.");
		OB_ASSERT((data != nullptr), "[VkAllocator] Invalid data passed in.");
		OB_ASSERT((size > 0), "[VkAllocator] Invalid size passed in for data.");
    ...
    {
		OB_PROFILE("VkAllocator::SetMappedData()");

		OB_ASSERT((mappedData != nullptr), "[VkAllocator] Invalid mapped data passed in.");
        OB_ASSERT((data != nullptr), "[VkAllocator] Invalid data passed in.");
        OB_ASSERT((size > 0), "[VkAllocator] Invalid size passed in for data.");
