...
namespace Obsidian::Internal
{
		...
		: m_Device(*api_cast<const Dx12Device*>(&device)), m_Specification(specs)
	{
		// Swapchain
		{
			...
			swapchainDesc.Flags = (m_Device.GetContext().AllowsTearing() ? DXGI_SWAP_CHAIN_FLAG_ALLOW_TEARING : 0);
				...
				OB_ASSERT(hwnd, "[Dx12Swapchain] Failed to retrieve win32 window, glfw error occurred.");
			#endif
	...
	{
		OB_PROFILE("Dx12Swapchain::AcquireNextImage()");
	...
	{
		OB_PROFILE("Dx12Swapchain::Present()");
		...
		if constexpr (Information::Validation)
		{
			OB_PROFILE("Dx12Swapchain::Present::Output");
	...
	{
		OB_ASSERT(m_CommandListFenceValues.contains(&commandList), "[Dx12Swapchain] CommandList is not known in current Swapchain.");
