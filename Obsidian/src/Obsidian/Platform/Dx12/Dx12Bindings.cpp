...
namespace Obsidian::Internal
{
    ...
    const BindingLayoutItem& Dx12BindingLayout::GetItem(uint32_t slot) const
    {
        ...
        OB_ASSERT(false, "[Dx12BindingLayout] Trying to retrieve a bindingitem from slot {0} which wasn't passed into the bindinglayout.", slot);
    ...
    void Dx12BindingSet::SetItem(uint32_t slot, Image& image, const ImageSubresourceSpecification& subresources, uint32_t arrayIndex)
    {
        ...
        OB_ASSERT(((item.Type == ResourceType::TextureSRV) || (item.Type == ResourceType::TextureUAV)), "[Dx12BindingSet] When uploading an image the ResourceType must be TextureSRV or TextureUAV.");
    ...
    void Dx12BindingSet::SetItem(uint32_t slot, Sampler& sampler, uint32_t arrayIndex)
    {
        ...
        OB_ASSERT((item.Type == ResourceType::Sampler), "[Dx12BindingSet] When uploading a sampler the ResourceType must be Sampler.");
    ...
    void Dx12BindingSet::SetItem(uint32_t slot, Buffer& buffer, const BufferRange& range, uint32_t arrayIndex)
    {
        ...
        case ResourceType::DynamicConstantBuffer:
        {
            OB_ASSERT((range.Size == BufferRange::FullSize), "[Dx12BindingSet] Dynamic buffers require a buffer range of FullSize.");
            OB_ASSERT((range.Offset == 0), "[Dx12BindingSet] Dynamic buffers require no buffer range offset.");
    ...
    {
        OB_ASSERT(((m_CurrentSet + 1) <= m_Specification.SetAmount), "[Dx12BindingSetPool] Using more bindingsets than specified in specification.");
