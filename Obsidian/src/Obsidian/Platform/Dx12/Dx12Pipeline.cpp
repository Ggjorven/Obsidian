...
namespace Obsidian::Internal
{
        ...
        : m_Specification(specs)
    {
        ...
        // Create root signature
        {
            ...
            for (auto& layout : m_Specification.BindingLayouts)
            {
                ...
                {
                    OB_ASSERT((ResourceTypeIsDynamic(type)), "[Dx12GraphicsPipeline] Internal error: A non dynamic resource type ended up in dynamic ranges.");
                ...
                if (m_PushConstantsIndex.Index == RootParameterIndices::Invalid) // Note: Only keep looking for pushconstants if not already found. // FUTURE TODO: Optimize
                {
                    for (const auto& item : dxLayout.GetBindingItems())
                    {
                        ...
                        OB_ASSERT((item.Size % 4 == 0), "[Dx12GraphicsPipeline] Push constants size must be aligned to 4 bytes.");
        ...
        {
            OB_ASSERT(m_Specification.Pass, "[Dx12GraphicsPipeline] No proper renderpass was passed in.");
            OB_ASSERT(m_Specification.Input, "[Dx12GraphicsPipeline] No proper input layout was passed in.");
    ...
    {
        OB_ASSERT((registerSpace < GraphicsPipelineSpecification::MaxBindings), "[Dx12GraphicsPipeline] Invalid registerspace passed in.");
    ...
    Dx12ComputePipeline::Dx12ComputePipeline(const Device& device, const ComputePipelineSpecification& specs)
        : m_Specification(specs)
    {
        OB_ASSERT(m_Specification.ComputeShader, "[Dx12ComputePipeline] To create a compute pipeline a compute shader must be passed in.");
    ...
    {
        OB_ASSERT((registerSpace < ComputePipelineSpecification::MaxBindings), "[Dx12ComputePipeline] Invalid registerspace passed in.");
        return m_RootParameterIndices[registerSpace].SRVAndUAVAndCBVIndices;
    }
