...
namespace Obsidian::Internal
{
	...
	const Dx12ImageSubresourceView& Dx12Image::GetSubresourceView(const ImageSubresourceSpecification& specs, ImageSubresourceViewUsage usage, ImageDimension dimension, Format format, bool isReadOnly)
	{
		...
		default:
			OB_ASSERT(false, "[VkImage] Trying to get imageview without index for SRV or UAV, these are regular DescriptorHeaps so dynamic indexing is not implemented.");
			break;
		}
	...
	const Dx12ImageSubresourceView& Dx12Image::GetSubresourceView(DescriptorHeapIndex index, const ImageSubresourceSpecification& specs, ImageSubresourceViewUsage usage, ImageDimension dimension, Format format)
	{
		...
		default:
			OB_ASSERT(false, "[VkImage] Trying to get imageview with index for RTV or DSV, these are DynamicDescriptorHeaps so manual indexing is not permitted.");
			break;
		}
	...
	Dx12StagingImage::SliceRegion Dx12StagingImage::GetSliceRegion(MipLevel miplevel, ArraySlice arraySlice) const
	{
		...
		OB_ASSERT((subresource < m_SubresourceOffsets.size()), "[Dx12StagingImage] Subresource index exceeds internal subresource offsets count.");
	...
	{
		UINT lastSubresource = CalculateSubresource(m_Specification.MipLevels - 1, m_Specification.ArraySize - 1, 0, m_Specification.MipLevels, m_Specification.ArraySize);
		OB_ASSERT((lastSubresource < m_SubresourceOffsets.size()), "[Dx12StagingImage] Subresource count doesn't add up.");
