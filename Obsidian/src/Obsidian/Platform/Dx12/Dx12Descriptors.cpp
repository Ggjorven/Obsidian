...
namespace Obsidian::Internal
{
	...
	{
		OB_ASSERT((m_Type == D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV), "[Dx12DescriptorHeap] Cannot allocate an SRV from a non SRV heap.");
		OB_ASSERT(resource, "[Dx12DescriptorHeap] Resource must not be null.");
		...
		{
			OB_ASSERT(specs.Size != 0, "[Dx12DescriptorHeap] Buffer size cannot be equal to 0.");
			OB_ASSERT(specs.Stride != 0, "[Dx12DescriptorHeap] Buffer stride cannot be equal to 0.");
		...
		//{
		//	OB_ASSERT(format != Format::Unknown, "[Dx12DescriptorHeap] ...");
		//
	...
	{
		OB_ASSERT((m_Type == D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV), "[Dx12DescriptorHeap] Cannot allocate an UAV from a non UAV heap.");
		OB_ASSERT(resource, "[Dx12DescriptorHeap] Resource must not be null.");
	...
	void Dx12DescriptorHeap::CreateCBV(DescriptorHeapIndex index, const BufferSpecification& specs, ResourceType type, ID3D12Resource* resource)
	{
		...
		OB_ASSERT((m_Type == D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV), "[Dx12DescriptorHeap] Cannot allocate a CBV from a non CBV heap.");
		OB_ASSERT(resource, "[Dx12DescriptorHeap] Resource must not be null.");
		OB_ASSERT((specs.Size % BufferSpecification::DefaultUniformBufferAlignment == 0), "Internal Error: [Dx12DescriptorHeap] Constant buffer size must be a aligned to DefaultUniformBufferAlignment.");
	...
	void Dx12DescriptorHeap::CreateCBV(DescriptorHeapIndex index, const D3D12_CONSTANT_BUFFER_VIEW_DESC& desc, ID3D12Resource* resource)
	{
		OB_ASSERT((m_Type == D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV), "[Dx12DescriptorHeap] Cannot allocate a CBV from a non CBV heap.");
		OB_ASSERT(resource, "[Dx12DescriptorHeap] Resource must not be null.");
		OB_ASSERT((desc.SizeInBytes % BufferSpecification::DefaultUniformBufferAlignment == 0), "Internal Error: [Dx12DescriptorHeap] Constant buffer size must be a aligned to DefaultUniformBufferAlignment.");
	...
	{
		OB_ASSERT((m_Type == D3D12_DESCRIPTOR_HEAP_TYPE_RTV), "[Dx12DescriptorHeap] Cannot allocate an RTV from a non RTV heap.");
		OB_ASSERT(resource, "[Dx12DescriptorHeap] Resource must not be null.");
	...
	{
		OB_ASSERT((m_Type == D3D12_DESCRIPTOR_HEAP_TYPE_DSV), "[Dx12DescriptorHeap] Cannot allocate an DSV from a non DSV heap.");
		OB_ASSERT(resource, "[Dx12DescriptorHeap] Resource must not be null.");
	...
	{
		OB_ASSERT((m_Type == D3D12_DESCRIPTOR_HEAP_TYPE_SAMPLER), "[Dx12DescriptorHeap] Cannot allocate a Sampler from a non Sampler heap.");
	...
	void Dx12DescriptorHeap::Grow(uint32_t minNewSize)
	{
		...
		if constexpr (Information::Validation)
		{
			...
			{
				OB_ASSERT((newSize < ((m_Type == D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV) ? static_cast<uint32_t>(D3D12_MAX_SHADER_VISIBLE_DESCRIPTOR_HEAP_SIZE_TIER_1) : static_cast<uint32_t>(D3D12_MAX_SHADER_VISIBLE_SAMPLER_HEAP_SIZE))), "[Dx12DescriptorHeap] Size of new descriptor heap exceeds maximum size.");
			}
		}
	...
	{
		OB_ASSERT(m_IsShaderVisible, "[Dx12DescriptorHeap] To be able to retrieve a GPU the heap must be shader visible.");
